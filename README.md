...
- VimTeX/latexmk and Autoformat policies; host-provided viewers/formatters. C/C++/CUDA build/clean/run/error-list keys (`<leader>m[dcre]`) and Python docstring/textobject keys are buffer-local and cleaned up on filetype changes.
...
- Optional Copilot/Octo/Git completion, Org/capture/calendar, task quickfix/floating views, marked-text-block scanning (`:ThesisAI`), notebook conversion, Markdown preview and remote sync. These are retained integrations, **not bundled accounts, documents or backends**.

### Host dependencies and explicit local configuration

Run `:PortableHealth` for configuration/dependency status. No credentials, language servers, parsers, interpreters, viewer binaries, clipboard providers or toolchains are automatically installed by the configuration or installer. Upstream manual installation commands remain explicit user actions.
...
Org requires explicit agenda/notes paths and a provisioned parser; capture templates, scratch/calendar paths, timezone parsing/formatting and synchronization backends remain local. Task providers return quickfix items or Markdown. Marked-block scanning requires explicit markers and a root. REPL startup takes executable argv and optional startup lines, not a guessed activation alias/session. Remote sync takes explicit local/remote paths and runs only on `ARsyncDown`, `ARsyncUp`, or `ARsyncUpDelete` (the latter deletes extraneous destination files). No project sync files are discovered or sourced automatically. The callable `RunPythonCode`, `LaunchMpv`, `SendToTmux` and `SendToScreen` helpers remain available through these same implementations: Python requires the tasks opt-in; standalone playback requires a media opt-in and player argv provider; session helpers require a media/calendar/REPL opt-in plus an explicit target. Arguments are passed without shell interpolation.
...
Without those prerequisites the extra parser proof is explicitly reported as not run; account/player calls remain instrumented, not live-account evidence.
